...
source(file.path("scripts", "helper_3phase_scripts.R"))
source(file.path("scripts", "helper_survival_scripts.R"))

# data_file <- file.path("/Users/ncamarda/Library/CloudStorage/OneDrive-Tufts/phd/ws/canineBP/data/2023-04-07 All Canine Data.xlsx")
# data_file <- file.path("data/Palladia dog data for Nick 7-13-23.xlsx")
data_file <- file.path("data/ToceranibTherapyInDo_DATA_LABELSv3.xlsx")

max_survival_cutoff_date <- ymd("2024-01-26")

# some parameters for the data
bp_index <- 1 # the sheet index of the redcap data in the excel spreadsheet, including BP
processed_data_dir <- file.path("data", "processed_data")
include_all_visits <- TRUE
six_months <- FALSE
my_pvalue_stars <- FALSE # use stars instead of the actual p-value in stats
my_remove_unscheduled_visits <- FALSE # remove unscheduled visits

# Phase block delineation
first_block <- c("baseline", "toceranib", "last_non_na_visit_toc")
second_block <- c("last_non_na_visit_toc", "anti_htn", "last_anti_htn")

phase_blocks <- enframe(c(first_block, second_block)) %>%
    mutate(block = c("first_block", "first_block", "first_block", "second_block", "second_block", "second_block")) %>%
    rename(phase_name = value) %>%
    dplyr::select(-name)

##########################################################################################################################################################################
##########################################################################################################################################################################
if (six_months) {
    data_dir <- file.path("data", "processed_data", "six_months_data")
    top_output_dir <- file.path("output", "all_visits", "six_months_results")
} else {
    data_dir <- file.path("data", "processed_data")
    top_output_dir <- file.path("output", "all_visits", "full_data")
}

survival_output_directory <- qq("@{top_output_dir}/survival_results")
...
# raw_data1 <- read_excel(file.path("data", "processed_data", "all_data_combined-processed-without_unscheduled_visits.xlsx"))
raw_data <- read_excel(file.path("data", "processed_data", "all_data_combined-processed-with_unscheduled_visits.xlsx")) %>%
    arrange(`Record ID`) %>%
...
# unique_sys_patients <- raw_data %>%
#     group_by(record_id, short_measurement) %>%
#     mutate(baseline_date = as_date(first(Date, order_by = Date))) %>%
#     filter(short_measurement == "systolic") %>%
#     remove_patients_with_no_baseline() %>%
#     pull(record_id) %>%
#     unique()
# n_distinct(unique_sys_patients)

extra_df <- read_excel(file.path("data", "processed_data", "canine_dosage_and_weight_info.xlsx")) %>%
    suppressWarnings() %>%
    group_by(`Record ID`) %>%
    summarize(
        avg_weight = mean(`Patient weight (kg)`, na.rm = TRUE),
        avg_dose_toc = mean(`Dosage of toceranib (mg/kg)`, na.rm = TRUE), .groups = "drop"
    )
...
baseline_data <- lapply(raw_data %>% split(.$short_measurement), FUN = function(x) {
    ...
    left_join(extra_df, by = join_by(`Record ID`)) %>%
    ungroup() %>%
    dplyr::select(-`Record ID`) %>%
    arrange(record_id)
# mutate(
#     baseline_date = if_else(drug != "toceranib", lag(absolute_last_visit_date, default = NA), baseline_date),
#
# ) %>%
# arrange(record_id) %>%
# ungroup()
...
survival_data <- read_excel("data/2024-01-19 Canine Patient Survival Data.xlsx") %>%
    ...
    dplyr::select(
        ...
        extracted_date, extracted_status, extracted_reason, final_htn_group, avg_weight, avg_dose_toc
    )
survival_data %>% print(n = Inf)
...
survival_data_summary <- survival_data %>%
    ...
    mutate(
        ...
        cutoff_date = max(baseline_data$absolute_last_visit_date, na.rm = TRUE), # max_survival_cutoff_date, Sudy's overall observation cutoff
        stop_date = if_else(
...
survival_data_prepared_all_cols <- survival_data_summary %>%
    mutate(
        # Determine what is most critical for your analysis—whether it's the effect of the
        # treatment up to the last point of clinical measurement or tracking survival/outcomes as
        # long as possible, regardless of the availability of specific clinical data.

        # this almost reproduces previous error, but coupled with the wrong censoring, i think this explains it:
        # if_else(!final_htn_group, as.numeric(stop_date - baseline_date), as.numeric(stop_date - baseline_date) * 2),
        # Calculate survival time from baseline_date to stop_date
...
survival_data_prepared_all_cols %>%
    rename(ttlv = time_to_last_visit, tst = total_survival_time) %>%
    filter(last_drug != "toceranib") %>%
    arrange(last_drug) %>%
    dplyr::select(record_id, last_drug, ttlv, tst, extracted_status, extracted_reason, last_known_alive_date, stop_date, final_status, cutoff_date) %>%
    print(n = Inf)
...
loli_surv_gg
ggsave(
    filename = file.path(survival_output_directory_plots, qq("@{basename(top_output_dir)}-survival_lollipop.pdf")),
    plot = loli_surv_gg,
    width = 9, height = 9
)
write.xlsx(survival_data_summary, file.path(survival_output_directory_data, qq("@{basename(top_output_dir)}-survival_data_with_baselines.xlsx")))
# glimpse(survival_data_prepared)
# survival_data_prepared %>%
#     print(n = Inf)
write.xlsx(survival_data_prepared_all_cols, file.path(survival_output_directory_data, qq("@{basename(top_output_dir)}-canine-processed_survival_data.xlsx")))
survival_data_prepared_all_cols
...
#' to account for lost follow-up. The analysis was stratified by treatment group (toceranib,
#'  toc + dox, toc + lis) to compare survival outcomes, incorporating the Kaplan-Meier method
#'  for survival curve estimation. P-values, trend tests, and cumulative events were calculated
#'  to assess statistical significance across treatment groups."
...
# survdiff(
#     survival::Surv(time = total_survival_time, event = final_status) ~ last_drug,
#     data = survival_data_prepared, rho = 1
# )
